...
class NACColorizedMultimodel(pl.LightningModule):
    ...
    def __init__(self, FC, dropout, backbone="ResNet50"):
        ...
        if stage==1:
            ...
            self.load_backbone = True
            # qua viene loadato solo il backbone. Al termine di questo training salva il checkpoint

        elif stage==2:
            self.freeze_backbone = False
            self.load_backbone = False
            # qua verrà loadato l'intero NAC model

        self.colorizer = None # MRIColorizer(type="pixelshuffle") #qua andrò a mettere il modulo colorizzato
        self.multiparametric = MultiParametricMRIModel(FC, dropout)  
    ...
    def forward(self, x: torch.Tensor) -> Dict:
        ...
        #prima devo colorizzare quelli giusti, poi devo riconcatenarli per darli in input al modello
        #multi che c'era già prima
...
class MultiParametricMRIModel(pl.LightningModule): #(nn.Module):
    ...
    def __init__(self, FC, dropout, backbone="ResNet50", freeze_backbone = False, freeze_classifiers = False):
        """
        ...
        self.branchDWI = BranchModel(self.backbone, "DWI", freeze_backbone)
        self.branchT2 = BranchModel(self.backbone, "T2",freeze_backbone)
        self.branchDCEpeak = BranchModel(self.backbone, "DCE_peak", freeze_backbone)
        self.branchDCE3TP = BranchModel(self.backbone, "DCE_3TP", freeze_backbone)
...
class BranchModel(pl.LightningModule): #(nn.Module):
    ...
    def __init__(self, backbone, branch_name, freeze_backbone: bool = False):
        ...
        # Freezing #TODO verificare se funziona
        if freeze_backbone:

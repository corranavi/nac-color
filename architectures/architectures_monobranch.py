'''This scripts provides the architectures for the model using a single branch with the overlapping of different MRI sequences.'''
...
class NACColorizedMONOmodel(pl.LightningModule):
    ...
    def __init__(self, FC, dropout, backbone="ResNet50", colorize=False, freeze_backbone = True, ckpt_filepath = "", evaluation=False):
        ...
        if self.colorize:
            self.colorizer = ColorizationModule(num_input_channels = 5)  # Monobranch is using 5 overlapping images

        self.multiparametric = MonobranchMRIModel(FC, dropout, backbone=backbone, freeze_backbone = freeze_backbone) 

    def forward(self, x: torch.Tensor) -> tuple[torch.tensor, Dict]:

        # Reference - The images are in the following sequence
        ...
        # ----- overlapping of the different MRI sequences -----
        ...
        # concatenate them so they are in the shape expected by the multiparametric
        colorized_x = torch.cat((x_PRE_colorized.unsqueeze(0), x_POST_colorized.unsqueeze(0)))
        
        # Pass input to the base classifier model
    ...
    def _reset_classifiers_layers(self):
        ...
        for n,modulo in enumerate(self.multiparametric.classifier._modules):
            print(f"{n} - {modulo}")
            if modulo in ["fc1", "fc2"]:    #dropout and activation function do not have parameters to be reset
                try:
                    self.multiparametric.classifier._modules[modulo].reset_parameters()
                    print(f"\tParameters of module {modulo} - reset done!")
...
class MonobranchMRIModel(pl.LightningModule):
    """
    Model for a single branch, implementing features extractor + classifier
    """
...
#         self.conv1 = nn.Conv2d(number_of_input_channels, 64, kernel_size=3, stride=1, padding=1)
